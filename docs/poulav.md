...
briefing, so they do not get renumbered. A ticked box means _the Rust client does this_.
...
_against_, never as work in progress.
...
offline suite is green: **190 passed, 2 intentionally ignored live-prover probes**.
...
- the TypeScript mirror remains an oracle rather than a production client; the Python seam
  and MCP production backend now speak protocol 2;
...
1. ~~**No prover endpoint.**~~ **Resolved.** Akash supplied a Sepolia
   ...
   failed at _execution_ (`-32603`), not at parsing, which means the service accepted our
   ...
   **So self-hosting is a product decision, not an MVP unblocker,** and the earlier text
   ...
   that is a demo convenience worth _stating_ rather than hiding. Doing it is Pathfinder
...
3. ~~**How we get a screening attestation.**~~ **CLOSED 2026-07-31, we never needed one.**
   ...
   whose allow response _is_ the STARK-curve signature over the depositor; the prover
   ...
   **How much this cost us is the part worth remembering.** F6 was opened 2026-07-25, and
   ...
   _The generalisable version, and it is not about screening:_ when a blocker is a question
...
- [x] **Get a Sepolia proving-service endpoint from StarkWare. GOT ONE 2026-07-30.** The
      ...
      `PATHFINDER_STORAGE_STATE_TRIES=10000`) is now a _product_ task, it is what makes the
...
- [ ] **Resolve screening, one experiment, do this before anything else in Phase 2.** Send a
      ...
      unpermissioned, class already declared). _Gates the shield only._ Cheapest possible
...
- [x] **Settlement ERC-20. DECIDED 2026-07-30: deploy our own mintable test ERC-20.**
      ...
      _Product note:_ because there is no allowlist, pointing this at USDC or STRK later is a
...
- [x] **Paymaster. DECIDED 2026-07-30: out of scope, post-MVP.** `get_fee_amount()` is 0,
      ...
      _Why it matters later, not now:_ "agents should not have to hold a gas token" is a real
...
_(Blocked on the prover endpoint.)_

### P0.2: Structured note payload: **answered: no payload field**

Payment-shaped envelope. Confirmed at three independent levels, the Cairo source, the
official docs (_"a note is an immutable record of three things"_: owner, token, amount), and
...
salt**. A subchannel is a _token_, not a topic, one per (channel, token), enforced. Full
...
30 minutes together, ARCHITECTURE §4 line by line. The file exists; the _agreement_ does
...
- [x] **`SettlementErrorCode`. FROZEN 2026-07-30.** Grounded in the errors that now exist
      rather than guessed, and grouped by what the caller should _do_, because an agent cannot
...
- [ ] **Protocol-2 disclosure correction needs shared sign-off.** Rust now returns a
      ...
      intentionally not edited in the Rust-only pass.
...
### P0.4: The Python ↔ Rust seam _(shared with Ishita)_
...
- [x] **Mechanism. DECIDED 2026-07-30: subprocess.** `erebus-cli`, JSON on stdin, JSON on
      ...
      keeps constraint 6 _structural_. In-process, "key material never leaves the SDK
...
- [x] **Rust protocol-2 seam landed 2026-07-30.** `erebus-cli` exposes all seven methods plus
      ...
      The Python transport and MCP adapter now speak this protocol-2 surface. Python only
      forwards arguments and reshapes results; protocol computation remains in Rust.
...
- [x] **`SettlementError` crossing. DONE.** A JSON envelope carries `code`, `message` and
      ...
      A broken install raises `SeamUnavailable` instead, intentionally a _different_ exception
...
- [x] Keep `/sdk/py` free of protocol logic. A hash, a felt conversion, or a salt encoder
      there is the bug this architecture is arranged to prevent.

**Acceptance:** protocol 2 crosses Rust -> JSON subprocess -> transport-only Python -> MCP
with typed results/errors, opaque state handles, note-balance reads, and self-contained
viewing grants.
...
- [x] Domain-separated hashes, all KAT'd _(first bug of the port lived here, tags truncated
      into a `u128`, caught in 30 seconds instead of a day. friction.md F12.)_
...
- [ ] **The `assert_valid_signature` route used by the pool**. `utils.cairo:383`
...
- [x] **`starknet_proveTransaction` client**, async on reqwest/rustls, retries only on
      ...
      _(`sdk/rs/src/prover.rs`)_. **Verified live against Sepolia:**
...
- [x] **Ordering and fee invariants as types**. `ActionSet` enforces phase order, the
      ...
      proof (~29 s) to discover. _Notable: a deposit alone reverts with `NO_REPLAY_PROTECTION`
      because `Deposit` emits no `WriteOnce`, you cannot shield without also creating a note._
...
_Phase 2 bullets, blocked on the prover:_
...
over the _sender's private key_, not a symmetric ECDH secret. The recipient cannot derive
...
- Fixed width: **5 notes per message**. Message _k_ is indices `5k .. 5k+4`, so the reader
...
- [x] Encoder/decoder for wire v2. 17 tests covering a pinned v2 KAT, round trips, tamper
      ...
      _(`sdk/rs/src/wire.rs`, vectors in `tests/fixtures/ts-wire-salts.json`,
      regenerate with `cd sdk/ts && pnpm vitest run tests/gen-wire-vectors.test.ts`)_.
...
- [x] Direct `ClientAction[]` construction. `Channel::write_message` emits five encrypted
      zero-amount notes at `5k..5k+4` as a validated `ActionSet` _(`sdk/rs/src/channel.rs`)_
...
- [x] **`PoolIdentity` holds the pool key with no accessor and a redacting `Debug`**. 11
      tests _(`tests/channel_ops.rs`)_. Constraint 6 becomes structural rather than
...
- [x] **DECIDED 2026-07-30: `withdrawn` is cut from the interface, on product grounds.**
      ...
      write-once, so an offer cannot be deleted; a retraction has to be a _new_ message, and
      ...
      voluntarily checks first. Atomic
...
- [ ] ~~**DECISION NEEDED (P0.3). `withdrawn` is unreachable.**~~ ARCHITECTURE §4 lists it as an
      ...
      prevent. `OfferStatus` in the Rust intentionally omits it rather than carrying a variant
...
- [x] `.env` chain id corrected from the human label `SN_SEPOLIA` to its felt encoding
      `0x534e5f5345504f4c4941`, which is the shape `erebus-cli` parses
...
deposits than Akash's. _Pushing work into Phase 1 is still right, and the reason is now
simply that on-chain iteration is slow at ~29 s a proof._

Beyond that, this phase is the Phase-1 tasks re-run for real: the bullets marked _Phase 2_
...
- [x] Bind the accepted offer to the private transfer in the same action set.
      `Channel::accept_and_settle` _(`sdk/rs/src/channel.rs`)_. Spends, then the payment
...
**Acceptance:** accept-and-settle succeeds atomically; a intentionally invalid proof leaves
state untouched.

**Live status:** successful atomic branch closed; intentionally invalid branch remains offline.
...
- [x] **A half grant is rejected, not partially disclosed.** Granting only the direction you
      ...
      required choice: for a compliance path, a record that quietly omits what the
...
different account with a different key sees nothing. _(This is DoD #3.)_
...
- Do not implement free-text messaging (ARCHITECTURE §7). The mechanism _could_ carry prose;
...
- If you find yourself refactoring the STRK20 primitives themselves, stop, we compose them,
  ...
  - **Tension named, not ignored:** the salt lane needs an SDK bypass (upstream hardcodes the
    note salt to `generateRandom120()` and drops it on read). That does not fork the _pool_,
