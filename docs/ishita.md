...
_salts_ of zero-amount notes in the counterparty's subchannel. One message is 4 notes and
...
_why_ you are truncating rather than just doing it: 128 bits is 2^64 collision
resistance. Fine for a commitment to a memo neither side is trying to forge, and the reason
the field is intentionally narrow rather than accidentally so.
...
grouped by _what you should do_, because an agent cannot sensibly branch on twelve codes:
...
- **Opaque:** `PROOF_FAILED`. The prover answers a failed execution with a bare
...
agent logic needs to branch on.
...
### I0.1: Agree the interface with Poulav _(do this first)_
...
- [x] **`acceptAndSettle` latency: ~30 s.** This is a measurement, not a guess.
      StarkWare publish ~29 s per transaction for Stwo proof generation. Per _transaction_,
...
- [x] Mock the _binding_ surface, not a hypothetical Python client. Whatever shape P0.4
...
- [x] Agent B (seller): has a reserve price; evaluates offers and leaves a seller-authored
      counter for the buyer to accept; it never accepts because the accepting identity pays
...
Done in `agents/src/erebus_agents/policy.py`, with 10 unit tests in `agents/tests/test_policy.py`.
...
Done. The path is `agents/src/erebus_agents/demo.py` (this repo uses a `src/` layout).

### I1.3: MCP server _(Python)_

This is what makes it _infrastructure_ rather than a demo. It's the difference between "we built a thing" and "any agent framework can use this."

Built on the official `mcp` Python SDK. `MCPServer`, decorator-defined tools, stdio transport
is enough for the demo.

- [x] MCP server exposing the seven protocol methods plus `wait_for_offers` and
      `get_note_balance` for exact-denomination planning
...
Done in `mcp-server/src/erebus_mcp/tools.py` and `server.py`, verified in
`mcp-server/tests/test_tools.py` against a real `mcp` SDK client over stdio, independent of
the reference agents. The production backend now reaches Rust through the protocol-2 seam;
the mock remains only for deterministic tests and policy rehearsal.

**Why this matters more than it looks:** StarkWare is evaluating whether this is reusable infrastructure. An MCP server that any framework can call _is_ the proof. Prioritize this over polishing the agents.

---

## The tool wire protocol: protocol 2, added 2026-07-31

**Read this before writing a tool handler.** The Rust CLI and the transport-only Python seam
now both speak protocol 2. Build against the contract below; the Rust request enum remains
the source of truth if this prose drifts.

The source of truth is `sdk/rs/src/bin/erebus_cli.rs:33`. If this section and that enum
disagree, the enum is right and this section is stale.
...
| -------------------- | ------------------------------------------------------------------- |
| `version`            | _(none, and no `config`: safe startup probe)_                      |
...
1. **Unknown fields are a hard error, not ignored** (`deny_unknown_fields`). A leftover
   ...
   `INVALID_REQUEST`. This protects the caller because a shape mismatch is the
...
3. **`reveal` takes no handle.** It takes the entire object `grant_viewing_key` returned, and
   that object _is_ the secret, anyone holding it can read that one relationship. Pass it
   ...
   machine with no access to our `state_dir`, as required by the compliance
   story in the demo.
...
- [x] `sdk/py/src/erebus/_seam.py` speaks protocol 2 and remains transport-only
- [x] The MCP production backend uses the real seam; the bundled policy demo intentionally
      remains a deterministic mock rehearsal
- [x] MCP writes run off the event loop and `wait_for_offers` handles real proof latency
- [x] Payer/payee roles and `get_note_balance` prevent reverse-direction settlement and
      agreements the payer's exact note denominations cannot settle

**Do not let this be the first time the two languages meet.** P0.4 already got one method
across with a stub underneath, so the _mechanism_ is proven, but protocol 2 then changed the
...
**Acceptance:** the scripted protocol-2 path is green on testnet. A corrected autonomous MCP
settlement still needs one fresh live run after the payer/payee guard landed.
...
5. **Starknet account abstraction docs**, https://docs.starknet.io. Specifically session keys and paymasters. This is _why_ agents work well here: no gas balance, no seed phrase management.
...
**Existing work:** the offer/counter/accept state machine is an agent decision loop. The x402 and ERC-8004 patterns use the same shape: autonomous agents transact under rules. The settlement rail is new.

**What does not transfer, checked 2026-07-28, so nobody plans around it:** the _code_.
...
settlement. Pattern transfers; libraries don't. _(If any of your prior x402 work is
on Starknet, say so. That would be TypeScript and changes this.)_
