...
#[test]
fn version_reports_protocol_five_and_wire_v3() {
    let (response, ok) = run(r#"{"method":"version"}"#);
    assert!(ok);
    assert_eq!(response["ok"], true);
    assert_eq!(response["protocol"], 5);
    assert_eq!(response["result"]["protocol"], 5);
