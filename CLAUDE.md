...
- **The seam is a subprocess**. `erebus-cli`, one JSON request on stdin and one envelope on
  ...
  Python binding speaks protocol 2 and refuses a mismatched CLI by its envelope tag; do
...
**Do not change this interface unilaterally.** Both sides must agree to a change first. An uncoordinated change breaks the other track's mock and the weekend's parallel work.
...
# Reference policy agents: deterministic mock rehearsal
uv run python agents/src/erebus_agents/demo.py

# MCP server (Python): use mock explicitly for local protocol tests. The settlement role
# is mandatory because accept_and_settle spends the caller's notes.
AGENT_ADDRESS=0xyouraddress PROVING_SERVICE_URL=http://placeholder \
  EREBUS_SETTLEMENT_ROLE=payer EREBUS_BACKEND=mock \
